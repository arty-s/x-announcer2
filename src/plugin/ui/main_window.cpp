...
void MainWindow::buildUi() {
    ...
    ImGui::GetIO().FontGlobalScale = static_cast<float>(announcer_->settings().windowScale);
...
void MainWindow::drawSettingsTab() {
    ...
    // Everything here writes straight into the live settings and takes effect
    ...
    section("ПРОЧЕЕ");
    {
        ...
        note("Пусто — плагин ищет сам. Сейчас читается:");
        const char* found = announcer_->simState().seatbeltDataref();
        ImGui::TextDisabled("  %s", found[0] == '\0' ? "этот борт табло не публикует" : found);
    ...
    {
        // The file is the other half of this tab: it holds the settings v2 does
        // not have controls for yet, and it is where a wrongly typed value is
        // explained. So its path is shown, not hidden.
        ...
        note("Плагин и так сохраняет через пару секунд после правки. "
             "Разбор файла с жалобами — во вкладке «Журнал».");
    }
}
